...
namespace App\Http\Controllers\Api\LinkIta\PDAM;
...
class PDAMController extends Controller
{
    ...
    public function InqCheck(Request $request)
    {
        ...
        $idtransaksi = $InqContent->id_transaksi_inq ?? null;
        if ($idtransaksi) {
            sleep(3);

            $checkInqRequest = $request;
            $checkInqRequest->merge(['idtransaksi' => $idtransaksi]);
